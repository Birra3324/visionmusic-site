...
function fmt(s){if(isNaN(s))return"0:00";const m=Math.floor(s/60);const sec=Math.floor(s%60);return`${m}:${sec.toString().padStart(2,"0")}`;}
...
function nextTrack(){if(isShuffle)currentIndex=Math.floor(Math.random()*queue.length);else currentIndex=(currentIndex+1)%queue.length;playTrack(currentIndex);}
function prevTrack(){if(audio.currentTime>3){audio.currentTime=0;return;}currentIndex=(currentIndex-1+queue.length)%queue.length;playTrack(currentIndex);}
...
function toggleQueue(){$("qpanel").classList.toggle("open");renderQueue();}
